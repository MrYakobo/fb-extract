...
function json2csv(myName, jsonfolder, output){
    const files = fs.readdirSync(jsonfolder).map(t=>path.join(jsonfolder,t))
...
`)
    sql.write(
`COPY users (name) FROM '${path}/users.csv' WITH csv;
COPY chats (chat_name) FROM '${path}/chats.csv' WITH csv;
COPY messages (cid, uid, content, media, timestamp) FROM '${path}/messages.csv' WITH csv;
COPY participants (uid, cid) FROM '${path}/participants.csv' WITH csv;
COPY reactions (mid, uid, emoji) FROM '${path}/reactions.csv' WITH csv;
`)
    sql.end()
